...
void main() {
  testWidgets('demo: tour-wide actions, suppressed on the first step', (
    tester,
  ) async {
    ...
    expect(
      texts(),
      contains('tooltipPosition.right'),
      reason: 'the first step tooltip is on screen',
    );

    // Step 0 is _topLeft, which the demo lists in hideActionsForShowcase.
    expect(
      find.byType(ShowCaseDefaultActions),
      findsNothing,
      reason: 'suppressed on the first step',
    );
    ...
    expect(
      texts(),
      contains('tooltipPosition.left'),
      reason: 'the tour advanced to the second step',
    );
    expect(
      find.byType(ShowCaseDefaultActions),
      findsOneWidget,
      reason: 'tour-wide actions reach it',
    );
